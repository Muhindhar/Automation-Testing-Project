...
public class PrescriptionAction extends BaseAction {
    ...
    public void clickIPDMenu() {
        ...
        wait.until(driver ->
                driver.findElement(
                        prescriptionPage.patientSearchBox
                ).isDisplayed()
        );
    }
    ...
    public void clickPrescriptionTab() {
        ...
        wait.until(driver ->
                driver.findElement(
                        prescriptionPage.addPrescriptionButton
                ).isDisplayed()
        );
    }
    ...
    public void clickAddPrescription() {
        ...
        wait.until(ExpectedConditions.presenceOfElementLocated(
                prescriptionPage.headerNote
        ));
    }

    // ─────────────────────────────────────────
    // Wysihtml5 iframe Helper
    // ─────────────────────────────────────────
    ...
    public void selectDoctorNotification() {
        try {
            WebElement prescribeBy = driver.findElement(
                    By.cssSelector("select[name='prescribe_by']"));

            String currentValue = (String) js.executeScript(
                    "return arguments[0].value;", prescribeBy);
            System.out.println("[selectDoctorNotification] prescribe_by value: '" + currentValue + "'");

            if (currentValue == null || currentValue.isEmpty()) {
                // Select Amit Singh — find by visible text
                Select sel = new Select(prescribeBy);
                for (WebElement opt : sel.getOptions()) {
                    System.out.println("  option: '" + opt.getText().trim() + "' = '" + opt.getAttribute("value") + "'");
    ...
    public void enterFindings(String value) {
        ...
        WebElement input = wait.until(
                ExpectedConditions.visibilityOfElementLocated(prescriptionPage.findings)
        );
        input.click();
        ...
        // Click the checkbox option that appears in the dropdown
        try {
            By option = By.xpath(
    ...
    private void setSelect2Value(By locator, String displayText) {
        ...
        String optionValue = (String) js.executeScript(
            ...
            "      sel.options[i].text.trim().indexOf(txt) !== -1) {" +
        ...
        System.out.println("[setSelect2Value] '" + displayText 
                + "' → value='" + optionValue + "'");
    ...
    public void selectMedicineCategory(String value) {
        ...
        System.out.println("[selectMedicineCategory] '"
                + value + "' → '" + optionValue + "'");

        // IMPORTANT: trigger both change + select2 events
        js.executeScript(
            "$(arguments[0]).val(arguments[1]);" +
            "$(arguments[0]).trigger('change');" +
            "$(arguments[0]).trigger('select2:select');",
            select,
            optionValue
        );

        // Wait until medicines actually load
        wait.until(driver -> {
        ...
        System.out.println(
                "[selectMedicineCategory] Medicine loaded successfully");
    ...
    public void selectMedicine(String value) {
        ...
        WebElement medicineDropdown = wait.until(
                ExpectedConditions.presenceOfElementLocated(
                        By.cssSelector("select[name='medicine_1']"))
        );

        // Wait a bit for AJAX medicine loading
        try {
            Thread.sleep(5000);
        } catch (InterruptedException ignored) {}
        ...
        java.util.List<WebElement> options =
                medicineDropdown.findElements(By.tagName("option"));
        ...
        if (matchedValue == null) {
            throw new RuntimeException(
                    "Medicine not found: " + value);
        }

        // Select using same JS style as category
        js.executeScript(
    ...
    public void selectDose(String value) {
        ...
        WebElement doseDropdown = wait.until(
                ExpectedConditions.presenceOfElementLocated(
        ...
        java.util.List<WebElement> options =
                doseDropdown.findElements(By.tagName("option"));
    ...
    public void clickSave() {
        try {
            ...
            WebElement saveButton = (WebElement) js.executeScript(
                "var buttons = document.querySelectorAll('button.btn-info');" +
        ...
        try {
            wait.until(driver -> {
                ...
                java.util.List<WebElement> alerts = driver.findElements(By.cssSelector(
                    ".alert-success, .toast-success, .growl-message, " +
        ...
        try {
            ...
            wait.until(
                    ExpectedConditions.visibilityOfElementLocated(
                            prescriptionPage.prescriptionRow
                    )
            );
